...
local function CreateClassificationMarkerSettings(containerParent, unit, updateCallback)
    local ClassificationMarkerDB = UUF.db.profile.Units[unit].Indicators.ClassificationMarker

    local ToggleContainer = GUIWidgets.CreateInlineGroup(containerParent, "Classification Marker Settings")

    local Toggle = AG:Create("CheckBox")
    Toggle:SetLabel("Enable |cFF8080FFClassification Marker|r Indicator")
    Toggle:SetValue(ClassificationMarkerDB.Enabled)
    Toggle:SetCallback("OnValueChanged", function(_, _, value) ClassificationMarkerDB.Enabled = value updateCallback() RefreshStatusGUI() end)
    Toggle:SetRelativeWidth(1)
    ToggleContainer:AddChild(Toggle)
    ...
    AnchorFromDropdown:SetList(AnchorPoints[1], AnchorPoints[2])
    AnchorFromDropdown:SetLabel("Anchor From")
    AnchorFromDropdown:SetValue(ClassificationMarkerDB.Layout[1])
    AnchorFromDropdown:SetRelativeWidth(0.5)
    AnchorFromDropdown:SetCallback("OnValueChanged", function(_, _, value) ClassificationMarkerDB.Layout[1] = value updateCallback() end)
    LayoutContainer:AddChild(AnchorFromDropdown)

    local AnchorToDropdown = AG:Create("Dropdown")
    AnchorToDropdown:SetList(AnchorPoints[1], AnchorPoints[2])
    AnchorToDropdown:SetLabel("Anchor To")
    AnchorToDropdown:SetValue(ClassificationMarkerDB.Layout[2])
    AnchorToDropdown:SetRelativeWidth(0.5)
    AnchorToDropdown:SetCallback("OnValueChanged", function(_, _, value) ClassificationMarkerDB.Layout[2] = value updateCallback() end)
    LayoutContainer:AddChild(AnchorToDropdown)

    local XPosSlider = AG:Create("Slider")
    XPosSlider:SetLabel("X Position")
    XPosSlider:SetValue(ClassificationMarkerDB.Layout[3])
    XPosSlider:SetSliderValues(-1000, 1000, 0.1)
    XPosSlider:SetRelativeWidth(0.33)
    XPosSlider:SetCallback("OnValueChanged", function(_, _, value) ClassificationMarkerDB.Layout[3] = value updateCallback() end)
    LayoutContainer:AddChild(XPosSlider)

    local YPosSlider = AG:Create("Slider")
    YPosSlider:SetLabel("Y Position")
    YPosSlider:SetValue(ClassificationMarkerDB.Layout[4])
    YPosSlider:SetSliderValues(-1000, 1000, 0.1)
    YPosSlider:SetRelativeWidth(0.33)
    YPosSlider:SetCallback("OnValueChanged", function(_, _, value) ClassificationMarkerDB.Layout[4] = value updateCallback() end)
    LayoutContainer:AddChild(YPosSlider)

    local SizeSlider = AG:Create("Slider")
    SizeSlider:SetLabel("Size")
    SizeSlider:SetValue(ClassificationMarkerDB.Size)
    SizeSlider:SetSliderValues(8, 64, 1)
    SizeSlider:SetRelativeWidth(0.33)
    SizeSlider:SetCallback("OnValueChanged", function(_, _, value) ClassificationMarkerDB.Size = value updateCallback() end)
    LayoutContainer:AddChild(SizeSlider)
...
local function CreateIndicatorSettings(containerParent, unit)
    local function SelectIndicatorTab(IndicatorContainer, _, IndicatorTab)
        ...
        elseif IndicatorTab == "Classification" then
            CreateClassificationMarkerSettings(IndicatorContainer, unit, function() UUF:UpdateUnitClassificationMarker(UUF[unit:upper()], unit) end)
        end
    end
